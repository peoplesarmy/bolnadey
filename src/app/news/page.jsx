  ...
  return (
    <div style={{ maxWidth: 1440, margin: '0 auto', padding: '110px 28px 80px', position: 'relative', zIndex: 1 }}>
      ...
      <form method="GET" style={{ display: 'flex', gap: 8, marginBottom: 24 }}>
        ...
        <button type="submit" style={{ padding: '12px 28px', background: 'linear-gradient(135deg,#FF0A16,#FF4D88)', border: 'none', borderRadius: 100, fontFamily: 'var(--font-outfit)', fontSize: 12, fontWeight: 800, color: 'white', cursor: 'pointer', whiteSpace: 'nowrap', letterSpacing: '.08em', textTransform: 'uppercase' }}>Search</button>
      </form>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 32, overflowX: 'auto', flexWrap: 'wrap' }}>
        {[{ label: 'All', value: '' }, ...CATEGORIES.map(c => ({ label: c, value: c }))].map(({ label, value }) => (
          <Link key={value} href={value ? `/news?category=${value}` : '/news'} style={{ padding: '7px 18px', background: activeCategory === value || (!activeCategory && !value) ? 'linear-gradient(135deg,#FF0A16,#FF4D88)' : 'rgba(255,255,255,.05)', border: `1px solid ${activeCategory === value || (!activeCategory && !value) ? 'transparent' : 'rgba(255,255,255,.08)'}`, borderRadius: 100, fontFamily: 'var(--font-outfit)', fontSize: 12, fontWeight: 700, color: activeCategory === value || (!activeCategory && !value) ? 'white' : 'var(--w3)', textDecoration: 'none', whiteSpace: 'nowrap', transition: 'all .2s', boxShadow: activeCategory === value || (!activeCategory && !value) ? '0 4px 20px rgba(255,10,22,.22)' : 'none' }}>{label}</Link>
        ))}
      </div>
      ...
      {pages > 1 && (
        <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginTop: 48 }}>
          {Array.from({ length: pages }, (_, i) => i + 1).map(p => (
            <Link key={p} href={`/news?${activeCategory ? `category=${activeCategory}&` : ''}page=${p}`} style={{ width: 40, height: 40, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: p === page ? 'linear-gradient(135deg,#FF0A16,#FF4D88)' : 'rgba(255,255,255,.05)', border: '1px solid rgba(255,255,255,.08)', fontFamily: 'var(--font-outfit)', fontSize: 13, fontWeight: 700, color: p === page ? 'white' : 'var(--w3)', textDecoration: 'none', boxShadow: p === page ? '0 4px 16px rgba(255,10,22,.22)' : 'none' }}>{p}</Link>
          ))}
        </div>
      )}
    </div>
